...
public interface IReservationRepository
{
    ...
    Task<Reservation?> GetPendingByUserAndBookAsync(
        UserId userId, BookId bookId, CancellationToken ct = default);
    Task<IReadOnlyList<Reservation>> GetPendingByBookAsync(
        BookId bookId, CancellationToken ct = default);
 
    Task<(IReadOnlyList<Reservation> Reservations, int TotalCount)> GetAllByUserAsync(
        UserId userId, int page, int pageSize, CancellationToken ct = default);
 
    Task<(IReadOnlyList<Reservation> Reservations, int TotalCount)> GetAllAsync(
