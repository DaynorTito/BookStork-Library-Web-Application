...
public sealed class LoanOverdueNotificationHandler : INotificationHandler<DomainEventWrapper<LoanOverdueEvent>>
{
    private readonly IEmailService _email;
    private readonly IUserRepository _userRepo;
    private readonly IBookRepository _bookRepo;
 
    public LoanOverdueNotificationHandler(IEmailService email, IUserRepository userRepo, IBookRepository bookRepo)
    { _email = email; _userRepo = userRepo; _bookRepo = bookRepo; }
 
    public async Task Handle(DomainEventWrapper<LoanOverdueEvent> n, CancellationToken ct)
    {
        ...
        if (user is not null && book is not null)
            await _email.SendLoanOverdueAsync(user.Email.Value, book.Name, evt.DueDate.Value, ct);
    }
}
